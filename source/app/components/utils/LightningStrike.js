...
class LightningStrike extends BufferGeometry {
	...
	init( rayParameters ) {
		...
		console.log('Type of simplexX:', typeof this.simplexX);
	...
	createMesh() {
		...
		// Populate the mesh
		this.fillMesh( 0 );
        console.log('LightningStrike: Initial fillMesh called. Vertices count:', this.currentVertex, 'Indices count:', this.currentIndex);
	...
	updateMesh( time ) {
		...
		this.fillMesh( time );
        console.log('LightningStrike: updateMesh called. Current time:', time, 'Draw range count:', this.currentIndex);
	...
	fillMesh( time ) {
		...
		this.fractalRay( time, ( segment ) => {
			...
			if ( time < subray.birthTime ) { //&& ( ! this.rayParameters.isEternal || scope.currentSubray.recursion > 0 ) ) {
			...
			} else if ( this.rayParameters.isEternal && this.currentSubray.recursion == 0 ) {
				...
				if ( this.timeFraction >= segment.fraction0 * subray.propagationTimeFactor ) {
				...
				if ( this.timeFraction <= subray.vanishingTimeFactor + segment.fraction1 * ( 1 - subray.vanishingTimeFactor ) ) {
	...
	fractalRayRecursive( segment ) {
		...
		// Noise
		this.newPos.set( this.simplexX( p.x, p.y, p.z, timeDimension ),
			this.simplexY( p.x, p.y, p.z, timeDimension ),
			this.simplexZ( p.x, p.y, p.z, timeDimension ) );
